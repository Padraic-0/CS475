...
main( int argc, char *argv[ ] )
{
	...
	//fprintf( stderr, "OpenMP is supported -- version = %d\n", _OPENMP );
		...
		// note: the Pin numbers don't need to be declared shared( ) because they are const variables!
                #pragma omp parallel for default(none) shared(holeaxs,holeays,holears, holebxs,holebys,holebrs, holecxs,holecys,holecrs, PinAx,PinAy,PinAr, PinBx,PinBy,PinBr, PinCx,PinCy,PinCr, stderr) reduction(+:numSuccesses)
                for( int n = 0; n < NUMTRIALS; n++ )
                {
